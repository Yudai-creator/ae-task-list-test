      ...
      <transition name="fade" mode="out-in">
        <div v-if="isClicked === true" id="add-task-actions" class="flex justify-between flex-wrap bg-gray-50 p-3">
          <div id="add-task-secondary-actions" class="flex flex-wrap lg:gap-8 gap-1">
            <div>
              <button :class="`lg:px-4 lg:py-2 p-1 ${taskInput != '' ? 'text-gray-600 bg-slate-200 cursor-pointer' : ' text-gray-200 bg-slate-100 cursor-default'} rounded-md flex items-center gap-2`">
          ...
          <div id="add-task-primary-actions" class="flex gap-2">
            <button @click="cancel" class="px-4 py-2 bg-slate-200 rounded-md lg:block hidden">
              Cancel
            </button>
